...
#'
#'The \code{\link{stratified}} function samples from a \code{\link{data.frame}}
#'in which one of the columns can be used as a "stratification" or "grouping"
#'variable. The result is a new \code{data.frame} with the specified number of
...
#'value between \code{0} and \code{1} expressed as a decimal, size is set to be
#'proportional to the number of observations per group. \item If \code{size} is
#'a single positive integer, it will be assumed that you want the same number
#'of samples from each group. \item If \code{size} is a vector, the function
#'will check to see whether the length of the vector matches the number of
#'groups and use those specified values as the desired sample sizes. The values
#'in the vector should be in the same order as you would get if you tabulated
#'the grouping variable (usually alphabetic order); alternatively, you can name
#'each value to ensure it is properly matched. }
...
#'\code{set.seed} each time that you go to take a sample from a different group
#'(in other words, the same seed is used for each group).
#'
#'The inclusion of a \code{seed} argument is mostly a matter of convenience, to
#'be able to have a single seed with which the samples can be verified later.
#'However, by using the \code{seed} argument, the same seed is used to sample
#'from each group. This may be a problem if there are many groups that have the
#'same number of observations, since it means that the same observation number
#'will be selected from each of those grops. For instance, if group "AA" and
#'"DD" both had the same number of observations (say, 5) and you were sampling
#'3 cases using a seed of 1, the second, fifth, and fourth observation would be
#'taken from each of those groups. To avoid this, you can set the seed using
#'\code{set.seed} \emph{before} you run the \code{\link{stratified}} function.
#'
#'As a user, you need to weigh the benefits and drawbacks of setting the seed
#'\emph{before} running the function as opposed to setting the seed \emph{with}
#'the function.  Setting the seed before would be useful if there are several
#'groups with the same number of observations; however, in the slim chance that
#'you need to verify the samples manually, you \emph{may} run into problems.
#'@author Ananda Mahto
#'@seealso \code{\link[sampling:strata]{strata}}
