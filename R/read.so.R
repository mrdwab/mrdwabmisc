...
#' 
#' The output of \code{\link{read.so}} is automatically assigned to an object
#' in your workspace called "\code{mydf}" unless specified using the \code{out}
#' argument.
#' 
#' @param sep Most of the time, the code shared is space separated (which is
#' the default for this function). If the separator is any other character, it
#' can be specified here.
#' @param header Are headers included?
#' @param out Desired output object name. Defaults to \code{mydf}.
#' @param stringsAsFactors Should strings be treated as \code{factor}s.
