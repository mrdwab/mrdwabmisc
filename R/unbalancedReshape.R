...
#'
#'Base R's \code{\link{reshape}} function cannot easily handle unbalanced wide data. \code{\link[reshape2:melt]{melt}}, from "reshape2" puts everything into a very long \code{data.frame}. This function returns a semi-long \code{data.frame} after automatically "balancing" the wide measures.
#'
...
#'@param dropNA Logical. If all the measure variables for some cases results in a row of \code{NA} values, shoud that row be dropped? Defaults to \code{TRUE}
#'@note Attributes from the output of \code{reshape} are retained, allowing you to easily re-convert to the wide form, if necessary.
#'@author Ananda Mahto
