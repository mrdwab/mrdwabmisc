...
#'
#'The \code{\link{reshape}} function in base R is very handy when you want a semi-long (or semi-wide) \code{data.frame}. However, base R's \code{reshape} has problems is with "unbalanced" panel data, for instance data where one variable was measured at three points in time, and another only twice.
#'
...
#'
#'Related functions like \code{\link{stack}} in base R and \code{\link[reshape2:melt]{melt}} in "reshape2" are also very handy when you want a "long" reshaping of data, but they result in a very long structuring of your data, not the "semi-wide" format that \code{reshape} produces.
#'
...
#'@param sep The character that separates the "variable name" from the "times" in the wide \code{data.frame}.
#'@param \dots Further arguments to \code{\link{NoSep}} in case the separator is of a different form.
#'@return A "long" \code{data.frame} of the reshaped data that retains the attributes added by base R's \code{reshape} function.
#'@author Ananda Mahto
#'@seealso \code{\link{Stacked}}, \code{\link{stack}}, \code{\link{reshape}}, \code{\link[reshape2:melt]{melt}}
#'@examples
#'
#'set.seed(1)
