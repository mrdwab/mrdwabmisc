...
#'@param cat Do you want "common" names, "rare" names, names with an "average"
#'frequency, or some combination of these? Should be specified as a character
#'vector (for example, \code{c("rare", "common")}). Defaults to \code{NULL}, in
#'which case all names are used as the sample frame.
#'@param gender Do you want first names from the "male" dataset, the "female"
...
#'@param MFprob What proportion of the sample should be male names and what
...
#'@param dataset What do you want to use as the dataset of names from which to
...
#'@note \emph{Dataset Details} This function samples from a provided dataset of
#'names. By default, it uses the data from the \emph{Genealogy Data: Frequently
#'Occurring Surnames from Census 1990--Names Files} web page. Those data have
#'been converted to \code{list} named \code{"CensusNames1990"} containing three
#'\code{data.frame}s (named \code{"surnames"}, \code{"malenames"}, and
#'\code{"femalenames"}).
#'
...
#'contents of each list item is a \code{data.frame} with at least the following
#'named columns: \code{"Name"} and \code{"Category"}. \item Acceptable values
#'for \code{"Category"} are \code{"common"}, \code{"rare"}, and
#'\code{"average"}. }
#'@author Ananda Mahto
