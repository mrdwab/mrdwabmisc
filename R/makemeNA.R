...
#'\code{\link{NA}}. One approach is to use \code{mydf[mydf == "some-character"]
#'<- NA}.  However, in many cases that results in a \code{data.frame} where
#'variables which should be numeric end up as characters or factors if the NA
#'string was a character to begin with. This function is a convenience wrapper
#'around \code{\link{type.convert}} to address such problems.
#'
#'
