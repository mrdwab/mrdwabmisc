...
#' \code{\link{concat.split.compact}}, \code{\link{concat.split.multiple}}, \code{\link{binaryMat}},
#' \code{\link{valueMat}}
#' @examples
#' 
#' temp <- head(concat.test)
...
#' @return A \code{data.frame} with the concatenated column split and added as a \code{list}.
#' @author Ananda Mahto
...
#' @param structure Can be either \code{"compact"}, \code{"expanded"}, or \code{"list"}. Defaults to
#' \code{"compact"}. See Details.
#' @param mode Can be either \code{"binary"} or \code{"value"} (where \code{"binary"} is default and it
#' recodes values to 1 or \code{NA}, like Boolean data, but without assuming 0 when
#' data is not available).  This setting only applies when \code{structure =
#' "expanded"}; a warning message will be issued if used with other structures.
#' @param drop Logical (whether to remove the original variable from the
...
#' @param fill The "fill" value for missing values when \code{structure = "expanded"}. Defaults to \code{NA}.
#' @note This is more of a "legacy" or "convenience" wrapper function encompassing the features available in the separated functions of \code{\link{concat.split.compact}}, \code{\link{concat.split.list}}, and \code{\link{concat.split.expanded}}. 
#' @author Ananda Mahto
...
#' \url{http://stackoverflow.com/q/10100887/1270695} for some history of this function, even though the solution is not used at all here. \item The
...
#' 
#' This is an extended version of the \code{\link{concat.split.compact}} function that allows the user to split multiple columns at once and optionally use the \code{\link{Reshape}} function to convert the \code{data.frame} into a "long" format.
#' 
#' @param data The source \code{data.frame}.
...
#' @param direction The desired form of the resulting \code{data.frame}, either \code{'wide'} or \code{'long'}. Defaults to \code{'wide'}.
#' @return A \code{data.frame}. If \code{direction = "long"}, a \code{data.frame} with additional attributes created by the \code{\link{reshape}} function in base R.
#' @author Ananda Mahto
#' @seealso \code{\link{concat.split}}, \code{\link{concat.split.compact}},
