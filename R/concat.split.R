...
#'@param split.col The variable that needs to be split; can be specified either by the column number or the variable name.
#'@param sep The character separating each value (defaults to ",").
#'@param structure Can be either "compact", "expanded", or "list". Defaults to "compact". See Details.
#'@param mode Can be either binary or value (where binary is default and it recodes values to 1 or NA, like Boolean data, but without assuming 0 when data is not available). This setting only applies when structure = "expanded"; an warning message will be issued if used with other structures.
#'@param drop.col Logical (whether to remove the original variable from the output or not). Defaults to TRUE.
#'@param fixed Is the input for the \code{sep} value \emph{fixed}, or a \emph{regular expression}? See Details.
#'
#'@details \emph{structure}
#'\itemize{
#'\item \code{"compact"} creates as many columns as the maximum length of the resulting split. This is the most useful general-case application of this function.
#'\item When the input is numeric, \code{"expanded"} creates as many columns as the maximum value of the input data. This is most useful when converting to \code{mode = "binary"}.
#'\item \code{"list"} creates a single new column that is structurally a \code{\link{list}} within a \code{\link{data.frame}}.
#'}
#'\emph{fixed}
#'When \code{structure = "expanded"} or \code{structure = "list"}, it is possible to supply a a regular expression containing the characters to split on. For example, to split on \code{","}, \code{";"}, or \code{"|"}, you can set \code{sep = ",|;|\\|"} or \code{sep = "[,;|]"}, and \code{fixed = FALSE} to split on any of those characters.
#'@note If using \code{structure = "compact"}, the value for \code{sep} can only be a single character. See the "Advanced Usage" example of how to specify multiple characters for batch conversion of columns.
#'@author Ananda Mahto
#'@references \itemize{
#'\item{See \url{http://stackoverflow.com/q/10100887/1270695}}
#'\item{The \code{"condensed"} setting was inspired by an answer from David Winsemius to a question at Stack Overflow. See: \url{http://stackoverflow.com/a/13924245/1270695}}
#'}
#'@examples
#'
#'## Load some data
