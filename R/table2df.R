...
#'
#'The \code{\link{table2df}} function takes an object of "class" \code{\link{table}}, \code{\link{ftable}}, and \code{\link{xtabs}} and converts them to \code{\link{data.frame}}s, while retaining as many of the name details as possible.
#'
#'@param mytable The \code{table} object you want to convert into a \code{data.frame}. This can be an object in your workspace, or you can make the call to \code{table}, \code{ftable}, or \code{xtabs} as the \code{mytable} argument to this function.
#'@param as.multitable Logical; defaults to \code{FALSE}. Some methods, for instance \code{xtabs} and \code{table}, will create an array of tables as the output when more than two variables are being tabulated.
#'\itemize{
#'\item If \code{as.multitable} is \code{TRUE}, the function will return a list of \code{data.frame}s. 
#'\item If \code{as.multitable} is \code{FALSE}, the function will convert the object to an \code{ftable} object before performing the transformation.
#'} 
#'@param direction Can be either \code{"long"} or \code{"wide"}. 
#'\itemize{
#'\item If \code{"long"}, the frequencies will all be tabulated into a single column. This is the same behavior you will generally get if you used \code{as.data.frame} on a \code{table} object.
#'\item If "wide", the tabular format is retained.
#'}
#'
#'@author Ananda Mahto
#'@seealso \code{\link{table}}, \code{\link{ftable}}, \code{\link{xtabs}}
#'@references The \code{expand.grid} method for remaking the columns from an \code{ftable} was described by Kohske at \url{http://stackoverflow.com/a/6463137/1270695}.
#'@examples
#'
#'# Make up some data:
#'set.seed(1)
