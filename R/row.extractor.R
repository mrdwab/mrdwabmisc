...
#'@param extract.by The column which will be used as the reference for
#'extraction; can be specified either by the column number or the variable
#'name.
#'@param what Options are \code{"min"} (for all rows matching the minimum
#'value), \code{"median"} (for the median row or rows), \code{"max"} (for all
#'rows matching the maximum value), or \code{"all"} (for min, median, and max);
#'alternatively, a numeric vector can be specified with the desired quantiles,
#'for instance \code{c(0, .25, .5, .75, 1)}.
#'@author Ananda Mahto
#'@seealso \code{\link{min}}, \code{\link{max}}, \code{\link{median}},
#'\code{\link{which.min}}, \code{\link{which.max}}, \code{\link{quantile}}
#'@references \itemize{ \item \code{which.quantile} function by cbeleites:
#'\url{http://stackoverflow.com/users/755257/cbeleites} \item See:
