...
#'
#'The \code{\link{multi.freq.table}} function takes a \code{\link{data.frame}} containing Boolean responses to multiple response questions and tabulates the number of responses by the possible combinations of answers. 
#'
#'In addition to tabulating the \emph{frequency} (\code{Freq}), there are two other columns in the output: \emph{Percent of Responses} (\code{Pct.of.Resp}) and \emph{Percent of Cases} (\code{Pct.of.Cases}). 
#'
...
#'@param sep The desired separator for collapsing the combinations of options; defaults to \code{""} (collapsing with no space between each option name).
#'@param boolean Are you tabulating boolean data (see \code{dat} Examples)? Defaults to \code{TRUE}.
#'@param factors If you are trying to tabulate non-boolean data, and the data are not factors, you can specify the factors here (see \code{dat2} Examples). Defaults to \code{NULL} and is not used when \code{boolean = TRUE}.
#'@param NAto0 Should \code{NA} values be converted to \code{0}? Defaults to \code{TRUE}, in which case, the number of valid cases should be the same as the number of cases overall. If set to \code{FALSE}, any rows with \code{NA} values will be dropped as invalid cases. Only applies when \code{boolean = TRUE}.
#'@param basic Should a basic table of each item, rather than combinations of items, be created? Defaults to \code{FALSE}.
#'@param dropzero Should combinations with a frequency of zero be dropped from the final table? Defaults to \code{TRUE}. Does not apply when \code{boolean = TRUE}.
#'@param clean Should the original tabulated data be retained or dropped from the final table? Defaults to \code{TRUE} (drop). Does not apply when \code{boolean = TRUE}.
#'
#'@author Ananda Mahto
#'@references \code{apply} shortcut for creating the \code{Combn} column in the output by Justin. See: \url{http://stackoverflow.com/q/11348391/1270695} and \url{http://stackoverflow.com/q/11622660/1270695}
#'
#'@examples
#'
#'## ========================================= ##
